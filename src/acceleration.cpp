...
void Acceleration::run(Eigen::MatrixXf localPath, cluon::data::TimeStamp sampleTime){
  ...
  if (localPath.rows()<=0 || (localPath.rows()==2 && (std::abs(localPath(0,0))<=0.00001f && std::abs(localPath(1,0))<=0.00001f && std::abs(localPath(0,1))<=0.00001f && std::abs(localPath(1,1))<=0.00001f))){
      ...
      if (!noPath) {
        ...
        if (localPath.rows()>1) {
          ...
          if (m_traceBack){ //TODO remove function if never used
            Eigen::RowVector2f firstPoint = Acceleration::traceBackToClosestPoint(localPath.row(0), localPath.row(1), Eigen::RowVector2f::Zero(1,2));
          ...
          localPath = Acceleration::placeEquidistantPoints(localPathCopy,false,-1,m_distanceBetweenPoints);
        } //else One point
      }
    }
    ...
    if (m_sharp) {
      ...
      headingRequest = Acceleration::driverModelSharp(localPath, previewDistance);
...
Eigen::VectorXf Acceleration::curveFit(Eigen::MatrixXf matrix)
{
  Eigen::VectorXf a;
  Eigen::MatrixXf M;
...
Eigen::RowVector2f Acceleration::traceBackToClosestPoint(Eigen::RowVector2f p1, Eigen::RowVector2f p2, Eigen::RowVector2f q)
{
...
Eigen::MatrixXf Acceleration::placeEquidistantPoints(Eigen::MatrixXf oldPathPoints, bool nEqPointsIsKnown, int nEqPoints, float eqDistance)
{
...
float Acceleration::driverModelSharp(Eigen::MatrixXf localPath, float previewDistance){
  float headingRequest;
  int n = m_nSharp;
...
std::vector<float> Acceleration::curvatureTriCircle(Eigen::MatrixXf localPath, int step){
...
std::vector<float> Acceleration::curvaturePolyFit(Eigen::MatrixXf localPath){
  int n = m_polyDeg;
